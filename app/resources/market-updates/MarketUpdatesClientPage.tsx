  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="space-y-8">
        <h2 className="text-2xl font-semibold text-coastal-blue">Monthly Snapshot – July 2025</h2>
        <p>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            <Card className="text-center p-6 hover:shadow-lg transition-shadow">
              <CardContent className="p-0">
                <div className="w-16 h-16 bg-coastal-blue/10 rounded-full flex items-center justify-center mx-auto mb-4">
                  <DollarSign className="h-8 w-8 text-coastal-blue" />
                </div>
                <h3 className="text-2xl font-bold text-coastal-navy mb-2">{currentStats.averagePrice}</h3>
                <p className="text-coastal-grey mb-2">Median Home Price</p>
            ...
            <Card className="text-center p-6 hover:shadow-lg transition-shadow">
              <CardContent className="p-0">
                ...
                <h3 className="text-2xl font-bold text-coastal-navy mb-2">{currentStats.salesVolume}</h3>
                <p className="text-coastal-grey mb-2">Homes Sold (Last 30 Days)</p>
                <div className="flex items-center justify-center space-x-1">
                  <TrendingUp className="h-4 w-4 text-green-500" />
                ...
                <CardContent className="p-8">
                  <div className="flex flex-col lg:flex-row gap-8">
                      ...
                      <div className="space-y-3 mb-6">
                        {report.highlights.map((highlight, highlightIndex) => (
                          <div key={highlightIndex} className="flex items-start space-x-3">
                            ...
                            <span className="text-coastal-grey">{highlight}</span>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="lg:w-1/3">
                      <div className="bg-coastal-navy/5 rounded-xl p-6">
                        <h4 className="text-lg font-bold text-coastal-navy mb-4">Key Statistics</h4>
                        <div className="space-y-4">
                          <div className="flex justify-between">
                            <span className="text-coastal-grey">Median Price:</span>
                            <span className="font-semibold text-coastal-navy">{report.keyStats.medianPrice}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-coastal-grey">Sales Volume:</span>
                            <span className="font-semibold text-coastal-navy">{report.keyStats.salesVolume}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-coastal-grey">Days on Market:</span>
                            <span className="font-semibold text-coastal-navy">{report.keyStats.daysOnMarket}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-coastal-grey">Price Change:</span>
                ...
                <CardContent className="p-8">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="text-right">
                      <div className="text-2xl font-bold text-coastal-blue">{neighborhood.medianPrice}</div>
                  ...
                  <div className="mb-6">
                    <div className="flex items-center space-x-4 mb-4">
                      <div className="flex items-center space-x-2">
                        <Calendar className="h-4 w-4 text-coastal-grey" />
                        <span className="text-coastal-grey">{neighborhood.daysOnMarket}</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 mb-4">
                      {neighborhood.highlights.map((highlight, highlightIndex) => (
                        <div key={highlightIndex} className="flex items-start space-x-3">
                          <div className="w-2 h-2 bg-coastal-blue rounded-full mt-2 flex-shrink-0"></div>
                          <span className="text-coastal-grey text-sm">{highlight}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-coastal-blue/5 rounded-lg p-4">
                    <h4 className="font-semibold text-coastal-navy mb-2">Investment Outlook:</h4>
                    <p className="text-coastal-grey text-sm">{neighborhood.investmentOutlook}</p>
                  </div>
                </CardContent>
              </Card>
            ))}
              ...
              <Card key={index} className="hover:shadow-lg transition-shadow">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <h3 className="text-xl font-bold text-coastal-navy font-serif">
                      {trend.trend}
                    </h3>
                    <Badge
                      className={`${
                  ...
                  <p className="text-coastal-grey mb-4">{trend.description}</p>
